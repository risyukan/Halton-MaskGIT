#!/usr/bin/bash
# FFN partial-update + KV cache: 两者同步 refresh, 都 N=2 (13/26 gated steps refresh)
# 跑完自动 append 一行到 results/halton_large384_kvcache_sweep.txt
set -euo pipefail
...
export HALTON_KV_REFRESH_N=2

LOG=/tmp/eval_fid_ffn2_kv2.log
echo "[$(date '+%F %T')] start FFN_N=2 KV_N=2 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "ffn_N2_kv_N2_sync_refresh" "$FID" "$IS" "$RT" "13/26 refresh, KV active=U_t∪U_{t-1}∪still-masked" \
  >> results/halton_large384_kvcache_sweep.txt

echo "[$(date '+%F %T')] done  FFN_N=2 KV_N=2  FID=$FID  IS=$IS  rt=$RT"
