...
# 每 4 个 gated step 全量刷新一次缓存 (约 7/26 refresh)。
...
LOG=/tmp/eval_fid_large384_layercache_alllayers_N4.log
echo "[$(date '+%F %T')] start layercache all-layers N=4 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "layercache_alllayers_N4" "$FID" "$IS" "$RT" "~7/26 refresh (25%)" \
  >> results/halton_large384_layercache_alllayers_sweep.txt

echo "[$(date '+%F %T')] done  layercache all-layers N=4  FID=$FID  IS=$IS  rt=$RT"
