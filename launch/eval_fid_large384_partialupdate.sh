...
# === 对照实验版: partial_update=True ===
# 与 launch/eval_fid_large384.sh 唯一区别 = 下面这行 export。
# 其余所有参数完全一致, 与 baseline (FID 2.541) 严格可比。
# 用法: bash launch/eval_fid_large384_partialupdate.sh
set -euo pipefail

cd /work/q-li/Halton-MaskGIT || exit 1

# 让 HaltonSampler.__call__ 在评估管线(不显式传 partial_update)下走 True 分支
