#!/usr/bin/bash
# Base-384 / cfg=0.7 sweep — partial_update=True, NO cache refresh (pure partial mode)
...
LOG=/tmp/eval_fid_base384_cfg07_partialupdate_norefresh.log
echo "[$(date '+%F %T')] start base384 cfg=0.7 no-refresh -> $LOG"
