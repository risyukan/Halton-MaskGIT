...
# 每 2 个 gated step 全量刷新一次缓存 (13/26 refresh)。
...
LOG=/tmp/eval_fid_large384_layercache_alllayers_N2.log
echo "[$(date '+%F %T')] start layercache all-layers N=2 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "layercache_alllayers_N2" "$FID" "$IS" "$RT" "13/26 refresh (50%)" \
  >> results/halton_large384_layercache_alllayers_sweep.txt

echo "[$(date '+%F %T')] done  layercache all-layers N=2  FID=$FID  IS=$IS  rt=$RT"
