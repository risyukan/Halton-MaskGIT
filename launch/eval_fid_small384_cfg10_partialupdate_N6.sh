#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 6 gated steps (5/26 refresh (19%))
...
export HALTON_CACHE_REFRESH_N=6
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N6.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=6 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N6" "$FID" "$IS" "$RT" "5/26 refresh (19%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=6  FID=$FID  IS=$IS  rt=$RT"
