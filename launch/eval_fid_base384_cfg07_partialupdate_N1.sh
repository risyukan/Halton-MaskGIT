#!/usr/bin/bash
# Base-384 / cfg=0.7 sweep — partial_update=True, refresh every 1 gated step
# (N=1 sanity check: every gated step refreshes => 等価于 full-update baseline)
...
LOG=/tmp/eval_fid_base384_cfg07_partialupdate_N1.log
echo "[$(date '+%F %T')] start base384 cfg=0.7 N=1 -> $LOG"
