#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 13 gated steps (2/26 refresh (8%))
...
export HALTON_CACHE_REFRESH_N=13
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N13.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=13 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N13" "$FID" "$IS" "$RT" "2/26 refresh (8%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=13  FID=$FID  IS=$IS  rt=$RT"
