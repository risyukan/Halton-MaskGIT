...
#   refresh every 2 gated steps (13/26 refresh).
# 与 eval_fid_large384_partialupdate_N2.sh 唯一差异 = 多开 HALTON_ATTN_CACHE=1:
#   attention 也走 active-only, inactive 位置用上一步 cached attn delta (不再写 0)。
# 对照基线 (FFN-only, 同 N) 见 results/halton_large384_partialupdate_sweep.txt:
#   baseline 2.5410 / N2 2.665 / norefresh 6.0475
...
LOG=/tmp/eval_fid_large384_attncache_N2.log
echo "[$(date '+%F %T')] start large384 attn-cache N=2 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "attncache_refresh_every_N2" "$FID" "$IS" "$RT" "13/26 refresh (50%)" \
  >> results/halton_large384_attncache_sweep.txt

echo "[$(date '+%F %T')] done  attn-cache N=2  FID=$FID  IS=$IS  rt=$RT"
