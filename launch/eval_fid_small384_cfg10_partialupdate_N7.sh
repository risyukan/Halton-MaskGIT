#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 7 gated steps (4/26 refresh (15%))
...
export HALTON_CACHE_REFRESH_N=7
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N7.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=7 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N7" "$FID" "$IS" "$RT" "4/26 refresh (15%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=7  FID=$FID  IS=$IS  rt=$RT"
