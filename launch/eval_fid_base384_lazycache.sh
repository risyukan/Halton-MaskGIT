#!/usr/bin/bash
# Base-384 / cfg=0.7 — LazyMAR Token Cache (HALTON_LAZY_CACHE=1)
#
...
#
# 用法:
#   REFRESH_N=2 bash launch/eval_fid_base384_lazycache.sh
set -euo pipefail
...
LAZY_START="${LAZY_START:-0}"                    # base depth=12 -> 全部层 = 0..11
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  ...
  >> results/halton_base384_lazycache_sweep.txt
