#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 1 gated step
# (N=1 sanity check: every gated step refreshes => 等价于 full-update baseline)
...
export HALTON_CACHE_REFRESH_N=1
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N1.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=1 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N1" "$FID" "$IS" "$RT" "26/26 refresh (100%, == baseline)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=1  FID=$FID  IS=$IS  rt=$RT"
