#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 4 gated steps (7/26 refresh (27%))
...
export HALTON_CACHE_REFRESH_N=4
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N4.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=4 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N4" "$FID" "$IS" "$RT" "7/26 refresh (27%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=4  FID=$FID  IS=$IS  rt=$RT"
