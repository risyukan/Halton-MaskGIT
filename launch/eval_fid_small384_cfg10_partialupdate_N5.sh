#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 5 gated steps (6/26 refresh (23%))
...
export HALTON_CACHE_REFRESH_N=5
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N5.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=5 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N5" "$FID" "$IS" "$RT" "6/26 refresh (23%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=5  FID=$FID  IS=$IS  rt=$RT"
