#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 9 gated steps (3/26 refresh (12%))
...
export HALTON_CACHE_REFRESH_N=9
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N9.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=9 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N9" "$FID" "$IS" "$RT" "3/26 refresh (12%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=9  FID=$FID  IS=$IS  rt=$RT"
