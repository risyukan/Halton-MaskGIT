#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 26 gated steps (1/26 refresh (4%))
...
export HALTON_CACHE_REFRESH_N=26
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N26.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=26 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N26" "$FID" "$IS" "$RT" "1/26 refresh (4%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=26  FID=$FID  IS=$IS  rt=$RT"
