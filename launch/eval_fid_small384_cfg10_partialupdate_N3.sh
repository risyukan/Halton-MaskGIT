#!/usr/bin/bash
# Small-384 / cfg=1.0 sweep — partial_update=True, refresh every 3 gated steps (9/26 refresh (35%))
...
export HALTON_CACHE_REFRESH_N=3
...
LOG=/tmp/eval_fid_small384_cfg10_partialupdate_N3.log
echo "[$(date '+%F %T')] start small384 cfg=1.0 N=3 -> $LOG"
...
printf "%-42s %-8s %-8s %-9s %s\n" \
  "partial_refresh_every_N3" "$FID" "$IS" "$RT" "9/26 refresh (35%)" \
  >> results/halton_small384_cfg10_partialupdate_sweep.txt

echo "[$(date '+%F %T')] done  N=3  FID=$FID  IS=$IS  rt=$RT"
