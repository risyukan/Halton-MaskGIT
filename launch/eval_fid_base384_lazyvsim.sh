#!/usr/bin/bash
# Base-384 / cfg=0.7 — LazyMAR 选点复刻 (HALTON_LAZY_VSIM=1)
#
# 与 launch/eval_fid_large384_lazyvsim.sh 逐项对齐, 唯一差别 = 模型尺寸
# (large 1024/24层 -> base 768/12层) 与 cfg_w (0.5 -> 0.7)。
...
# base 只有 12 层, 同一语义 = L3-11。层 0..2 每步全量, 给入口层打分攒材料。
#
# 用法:
#   REFRESH_N=2 bash launch/eval_fid_base384_lazyvsim.sh
set -euo pipefail
...
LAZY_END="${LAZY_END:-11}"                       # base depth=12 -> 一直用到最后一层
...
LOG="/tmp/eval_fid_base384_${TAG}.log"
echo "[$(date '+%F %T')] start base384 ${TAG} -> $LOG"
...
printf "%-46s %-8s %-9s %-9s %s\n" \
  ...
  >> results/halton_base384_lazyvsim_sweep.txt
