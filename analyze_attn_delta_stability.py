"""
Quantify ATTENTION-delta stability across Halton-MaskGIT decoding steps.

This is the attention twin of analyze_ffn_delta_stability.py.  It answers the
question that the existing FFN-cache (Block.cached_ffn_delta) raised but never
verified for attention:

    "The current partial-update Attention writes ZERO into inactive positions
     (Network/transformer.py:145).  Would caching the previous step's attention
     output delta — mirroring the FFN cache — be a better fill than zero?"

For vanilla (full-token-update) inference, at every step t and every transformer
layer l we record the attention output delta per token:

    attn_delta_l(t) = alpha1 * attn( modulate(ln1(x), gamma1, beta1) )

i.e. exactly the term added to the residual stream by the attention sublayer
(post-wo, post-gate), BEFORE the FFN sublayer runs.  Then for each token that is
*inactive* at step t (NOT newly released by the Halton schedule) we compute:

    cos_l(t)   = cosine( attn_delta_l(t), attn_delta_l(t-1) )
    drift_l(t) = || attn_delta_l(t) - attn_delta_l(t-1) ||_2 / || attn_delta_l(t) ||_2

Stats are grouped by layer depth / step progress / token age, and inactive
tokens are split into the same two sub-populations as the FFN study:
  (a) committed-before-t: token id fixed, only context drifts  ← the cache target
  (b) still-masked:       token is mask_value at both t and t-1

Interpretation guide:
  - High cos & low drift in the mid-stack (layers ~3-21) for committed tokens
    ⇒ caching attn_delta is well-justified there; switching inactive fill from
      zero to cached-delta should help with little risk.
  - Compare the committed-inactive curves here against ffn_delta_stability.png:
    if attn_delta is *less* stable than ffn_delta, that explains why the
    original code chose zero for attention but cached-delta for FFN, and warns
    that an attn cache will need periodic refresh (HALTON_CACHE_REFRESH_N).

Pure runtime monkey-patch — does NOT modify Network/transformer.py,
Sampler/halton_sampler.py, or the FFN-cache logic.

Outputs:
  statics/attn_delta/attn_delta_stability.npz   raw per-token cos / drift arrays
  statics/attn_delta/attn_delta_stability.png   6-panel grouped plot
  statics/attn_delta/attn_delta_heatmap.png     layer x step heatmaps
"""
...
def _patched_block_forward(self, x, cond, mask=None, active_idx=None):
    ...
    # Attention sublayer — capture the exact residual increment.
    attn_delta = alpha1.unsqueeze(1) * self.attn(
    ...
    # FFN sublayer — unchanged full-token update so sampling stays faithful.
...
def parse_args():
    ...
    p.add_argument("--out-dir",    type=str, default="statics/attn_delta")
    return p.parse_args()
...
def aggregate_and_plot(cos, drift, commit_step, active_at, out_dir):
    ...
    ax.set_xlabel("layer index"); ax.set_ylabel("cos(attnδ_l(t), attnδ_l(t-1))")
    ax.set_title("attn-delta cosine vs prev step — by layer")
    ...
    ax.set_title("attn-delta cosine vs prev step — by step")
    ax.legend(); ax.grid(True, alpha=0.3)
    ...
    ax.set_title("attn-delta cosine — by token age (committed)")
    ax.grid(True, alpha=0.3)

    ax = axes[1, 0]
    ax.plot(drf_l_c, marker="o", label="committed inactive")
    ax.plot(drf_l_m, marker="s", label="still-masked inactive")
    ax.set_xlabel("layer index"); ax.set_ylabel("‖Δ(attnδ)‖₂ / ‖attnδ(t)‖₂")
    ax.set_title("attn-delta relative drift — by layer")
    ax.legend(); ax.grid(True, alpha=0.3)
    ...
    ax.set_title("attn-delta relative drift — by step")
    ax.legend(); ax.grid(True, alpha=0.3)

    ax = axes[1, 2]
    ax.plot(ages, drf_by_age, marker="o")
    ax.set_xlabel("token age"); ax.set_ylabel("rel drift")
    ax.set_title("attn-delta relative drift — by token age (committed)")
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    p_main = os.path.join(out_dir, "attn_delta_stability.png")
    ...
    axes[0].set_title("mean attn-delta cosine — committed inactive tokens")
    fig.colorbar(im, ax=axes[0])

    im = axes[1].imshow(heat_drift.T, aspect="auto", origin="lower", cmap="magma")
    axes[1].set_xlabel("step (t)"); axes[1].set_ylabel("layer")
    axes[1].set_title("mean attn-delta rel-drift — committed inactive tokens")
    fig.colorbar(im, ax=axes[1])

    plt.tight_layout()
    p_heat = os.path.join(out_dir, "attn_delta_heatmap.png")
    plt.savefig(p_heat, dpi=120); plt.close(fig)
    print("[plot] saved", p_heat)

    # ---------- print quick text summary ----------
    print("\n=== Summary (committed-inactive tokens, ATTENTION delta) ===")
    ...
    # Gate-region (layer 3-21, step 5-30) committed-inactive average — the region
    # the partial-update scheme actually uses; this is the number that decides
    # whether attn-delta caching is viable where it would be applied.
    gl0, gl1, gs0, gs1 = 3, min(22, L), 5, min(31, T)
    gate = bcast(inactive_committed)[gs0:gs1, gl0:gl1] & valid[gs0:gs1, gl0:gl1]
    if gate.any():
        gcos = cos[gs0:gs1, gl0:gl1][gate].mean()
        gdrf = drift[gs0:gs1, gl0:gl1][gate].mean()
        print(f"[gate l{gl0}-{gl1-1} s{gs0}-{gs1-1}] mean cos={gcos:.4f}  mean rel-drift={gdrf:.4f}")
...
def main():
    ...
    out_npz = os.path.join(args_cli.out_dir, "attn_delta_stability.npz")
    np.savez_compressed(
        out_npz,
